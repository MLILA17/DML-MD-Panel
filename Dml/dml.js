...
const { TelegraPh, UploadFileUgu } = require("../lib/toUrl.js");
const uploadtoimgur = require('../lib/Imgur.js');
const { commands, aliases, totalCommands } = require('../Handler/commandHandler.js');
const status_saver = require('../Functions/status_saver.js');
const gcPresence = require('../Functions/gcPresence.js');
const antitaggc = require('../Functions/antitag.js');
const antilink = require('../Functions/antilink.js');
const chatbotpm = require('../Functions/chatbotpm.js');
const { getSettings, getSudoUsers, getBannedUsers, getGroupSettings, updateSetting } = require('../Database/config.js');
const { botname, mycode } = require('../Env/settings.js');
const { cleanupOldMessages } = require('../lib/Store.js');
const antistatusmention = require('../Functions/antistatusmention.js');

const ownerMiddleware = require('../utility/botUtil/Ownermiddleware.js');
...
module.exports = Dml = async (client, m, chatUpdate, store) => {
    try {
        const sudoUsers = await getSudoUsers();
        const bannedUsers = await getBannedUsers();
        let settings = await getSettings();
        if (!settings) {
            console.error("DML-MD: Settings not found, cannot proceed!");
            return;
        }
        ...
        var msgDml = m.message?.extendedTextMessage?.contextInfo?.quotedMessage || null;
        var budy = typeof m.text == "string" ? m.text : "";

        const timestamp = speed();
        const Dmlspeed = speed() - timestamp;
        const filePath = require('path').resolve(__dirname, '../Dml.jpg');
        ...
        const DevDml = Array.isArray(sudoUsers) ? sudoUsers : [];
        const Owner = DevDml.map((v) => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net").includes(m.sender);
        ...
        const context = {
            ...
            pushname, body, budy, totalCommands, args, mime, qmsg, msgDml, botNumber, itsMe,
            packname, generateProfilePicture, groupMetadata, Dmlspeed, mycode, fetchJson, exec,
        ...
        console.error('❌ [Dml] Error:', err.message);
    }
